...
func setupFailureTestEngineer(t *testing.T) (*Engineer, *beads.Beads) {
	t.Helper()
	setupTestRegistry(t)
	...
	if err := os.MkdirAll(filepath.Join(rigPath, "refinery", "rig"), 0o755); err != nil {
	...
	e.beads = b
	e.output = io.Discard
	e.mergeSlotEnsureExists = func() (string, error) { return "merge-slot", nil }
...
func TestHandleMRInfoFailure_ReopensSourceIssueAndClosesMR(t *testing.T) {
	e, b := setupFailureTestEngineer(t)

	sourceIssue, err := b.Create(beads.CreateOptions{
		Title:    "Source issue",
		Type:     "task",
		Priority: 1,
	})
	...
	closed := "closed"
	if err := b.Update(sourceIssue.ID, beads.UpdateOptions{Status: &closed}); err != nil {
		t.Fatalf("close source issue: %v", err)
	}

	agentID := beads.AgentBeadIDWithPrefix("xut", "testrig", "polecat", "toast")
	if _, err := b.CreateAgentBead(agentID, "Polecat Toast", &beads.AgentFields{
		RoleType:   "polecat",
		Rig:        "testrig",
		AgentState: "working",
		ActiveMR:   "pending",
	...
	mrFields := &beads.MRFields{
		Branch:      "polecat/toast/oc-123",
		Target:      "main",
		SourceIssue: sourceIssue.ID,
		Worker:      "polecats/toast",
		Rig:         "testrig",
		AgentBead:   agentID,
	}
	...
	if err := b.UpdateAgentActiveMR(agentID, mrIssue.ID); err != nil {
		t.Fatalf("set agent active MR: %v", err)
	}

	e.HandleMRInfoFailure(&MRInfo{
		ID:          mrIssue.ID,
		...
		Success:     false,
		TestsFailed: true,
		Error:       "unit tests exploded",
	})

	gotMR, err := b.Show(mrIssue.ID)
	if err != nil {
		t.Fatalf("show MR: %v", err)
	}
	if gotMR.Status != "closed" {
		t.Fatalf("MR status = %q, want closed", gotMR.Status)
	}
	gotMRFields := beads.ParseMRFields(gotMR)
	if gotMRFields == nil || gotMRFields.CloseReason != "rejected" {
		t.Fatalf("MR close_reason = %+v, want rejected", gotMRFields)
	}

	gotSource, err := b.Show(sourceIssue.ID)
	if err != nil {
		t.Fatalf("show source issue: %v", err)
	}
	if gotSource.Status != "open" {
		t.Fatalf("source issue status = %q, want open", gotSource.Status)
	}
	if gotSource.Assignee != "" {
		t.Fatalf("source issue assignee = %q, want empty", gotSource.Assignee)
	}

	gotAgent, err := b.Show(agentID)
	if err != nil {
		t.Fatalf("show agent bead: %v", err)
	}
	agentFields := beads.ParseAgentFields(gotAgent.Description)
	if agentFields.ActiveMR != "" {
		t.Fatalf("agent active_mr = %q, want empty", agentFields.ActiveMR)
	}
}

func TestHandleMRInfoFailure_ConflictUpdatesMRMetadata(t *testing.T) {
	e, b := setupFailureTestEngineer(t)

	sourceIssue, err := b.Create(beads.CreateOptions{
		Title:    "Conflicted source issue",
		Type:     "task",
		Priority: 2,
	})
	if err != nil {
	...
	if err := b.Update(mrIssue.ID, beads.UpdateOptions{Assignee: &assignee}); err != nil {
		t.Fatalf("assign MR: %v", err)
	}
	...
	if gotMR.Status != "open" {
		t.Fatalf("MR status = %q, want open", gotMR.Status)
	}
	if gotMR.Assignee != "" {
		t.Fatalf("MR assignee = %q, want empty", gotMR.Assignee)
	}
	gotMRFields := beads.ParseMRFields(gotMR)
	if gotMRFields == nil {
		t.Fatal("expected MR fields")
	}
	if gotMRFields.RetryCount != 3 {
		t.Fatalf("retry_count = %d, want 3", gotMRFields.RetryCount)
	}
	if gotMRFields.ConflictTaskID == "" {
		t.Fatal("expected conflict_task_id to be set")
	}
}
